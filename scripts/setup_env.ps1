...
# 1b. Git LFS — fetch the large model checkpoints (*.pt) ---------------
Write-Host "==> Setting up Git LFS (for trained model files)..." -ForegroundColor Cyan
git lfs version 2>$null
if ($LASTEXITCODE -eq 0) {
    git lfs install
    git lfs pull        # download the actual .pt models (not just pointers)
} else {
    Write-Host "    Git LFS not found. The .pt model files will be 130-byte pointers." -ForegroundColor Yellow
    Write-Host "    Install it from https://git-lfs.com then run:  git lfs install; git lfs pull" -ForegroundColor Yellow
    Write-Host "    (Not required to RUN the pipeline — models can be regenerated by training.)" -ForegroundColor Yellow
}
